...
class Word_Recognizer:
    ...
    def compare_with_target(
        self, recognized_OCR, target_words, mode="token_set_ratio", threshold=80
    ):
        ...
        recognized_OCR = recognized_OCR.replace("\n", " ")

        target_text = " ".join(target_words)

        # print("recognized_OCR", recognized_OCR)
        # print("target_text", target_text)

        ratio_function = self.ratio_functions[mode]
        match_ratio = ratio_function(recognized_OCR, target_text)

        is_match = match_ratio >= threshold

        return is_match, match_ratio

    def compare_with_target_all_types(self, recognized_OCR, target_words, threshold=80):
        recognized_OCR = recognized_OCR.replace("\n", " ")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        my_name_OCR = pokeMMO.get_text_from_center(
            box_width=125, box_height=25, offset_x=0, offset_y=104, config="--psm 7"
        )  # Player Name and guild name
        # is_match, match_ratio = pokeMMO.word_recognizer.compare_with_target(
        #     my_name_OCR, target_words=target_words_dict["name_area"], threshold=60
        # )
        # print(
        #     f"my_name_OCR: {my_name_OCR}, is_match: {is_match}, match_ratio: {match_ratio}"
        # )
