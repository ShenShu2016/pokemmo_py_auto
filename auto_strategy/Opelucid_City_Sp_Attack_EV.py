...
def add_x_y_coords_offset_Opelucid_City_Sp_Attack(coords_status):
    print(coords_status, "add_x_y_coords_offset_Opelucid_City_Sp_Attack")
...
class Farming_Opelucid_City_Sp_Attack:
    """地图有点问题，但是问题不大"""

    def __init__(self, pokeMMO_instance: PokeMMO):
        self.p = pokeMMO_instance
        self.city = "Opelucid_City_Sp_Attack"
        self.my_df = self.p.df_dict["Opelucid_City_Sp_Attack_coords_tracking_csv"]
        self.farming_coords = [
            (x, y)
            for x, y, mark in zip(
    ...
    def run(self, repeat_times=10):
        ...
        while self.p.auto_strategy_flag:
            ...
            while self.p.get_gs()["return_status"] < 20:
                ...
                coords_status = add_x_y_coords_offset_Opelucid_City_Sp_Attack(
                    self.p.get_coords()
                )
...
if __name__ == "__main__":
    ...
    pokeMMO = PokeMMO()
    farming = Farming_Opelucid_City_Sp_Attack(pokeMMO)
    farming.run()
